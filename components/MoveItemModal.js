...
export default function MoveItemModal({ item, folderId, folders, show }) {
    ...
    return (
        <View style={[common.modalBg, { flexDirection: 'row' }]}>
            <View style={[common.modal, { backgroundColor: appThemeColor.modal }, { maxHeight: '50%' }]}>
                <View style={[styles.fxr, { flexDirection: 'row' }]}>
                    <Text style={[common.text, { color: appThemeColor.text }, { fontWeight: 'bold', }]}>{appLang == 0 ? '아이템 이동' : 'Move Item'}</Text>
                </View>
                ...
                <ScrollView
                    ...
                    {folders.map((folder, index) => (
                        <ModalButton key={index} idx={index} click={move} text={folder.title}></ModalButton>
                    ))}
