...
export function create(params) {
  db.transaction((tx) => {
    ...
    tx.executeSql(
      "CREATE TABLE IF NOT EXISTS folders (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, opened INTEGER default 1, itemOrder INTEGER);",
      null,
      (txObj, rs) => {
...
export function init(dispatch) {
  db.transaction((tx) => {
    tx.executeSql(
      `select * from folders order by itemOrder`,
      null,
      (txObj, rs) => {
        let items = [];
...
export function add(title, dispatch) {
  db.transaction(
    (tx) => {
      ...
      let date = new Date();
      tx.executeSql(
        `insert into folders (title, itemOrder) values(?,?)`,
        [title, date.getTime()],
        (txObj, rs) => {
...
export function fold(folder, dispatch) {
  ...
  db.transaction((tx) => {
    ...
    tx.executeSql(
      `update folders set opened=? where id=?`,
      [change, folder.id],
      (txObj, rs) => {
        console.log("update success");
        init(dispatch);
          ...
          for (var i = 0; i < len; i += 1) {
            ...
            txObj.executeSql(
              ...
              (txObj, rs) => {
                ...
                txObj.executeSql(
                  `delete from quicks where itemId=?`,
                  [itemId],
                  (txObj2, rs2) => {
  ...
  db.transaction(
    (tx) => {
      folders.forEach((folder, index) => {
        const id = folder.id;
        tx.executeSql(
          `update folders set itemOrder=? where id=?`,
          [index, id],
          (txObj, rs) => {},
          (txObj, err) => {}
        );
